...
def calculate_stats(img):
    ...
    specific_weights = []
    for area in areas:
        weights.append(np.sum(area))
        specific_weights.append(np.sum(area) / (w // 2 * h // 2))
    ...
    return {
        'weights (top_left, top_right, bottom_right, bottom_left)': weights,
        'spec_weights (top_left, top_right, bottom_right, bottom_left)': specific_weights,
        'center_of_mass': com,
        'normalized_center_of_mass': norm_com,
        'inertia': i,
        'normalized_inertia' : norm_i
    }
...
def write_stats(out_path):
    with open(f'{out_path}data.csv', 'w', newline='') as csvfile:
        fieldnames = [
            'weights (top_left, top_right, bottom_right, bottom_left)',
            'spec_weights (top_left, top_right, bottom_right, bottom_left)',
            'center_of_mass',
            'normalized_center_of_mass',
            'inertia',
            'normalized_inertia',
        ]
...
def main():
    ...
    generate_symbol_imgs('./input/TimesNewRomanRegular.ttf', './output/inverted_symb/', inverted=True)
    out_path = './output/'
